...
export const Login = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!correo.trim()) {
      newErrors.correo = rol === "estudiante"
        ? "Ingrese su correo institucional."
        : "Ingrese el correo de contacto de la empresa.";
      isValid = false;
    }

    if (rol === "estudiante" && correo.trim() && !correo.trim().endsWith("@unmsm.edu.pe")) {
    ...
    if (!password.trim()) {
      newErrors.password = rol === "estudiante"
    ...
    try {
      ...
      if (rol === "estudiante") {
        ...
        const estudiante = estudiantes.find(
          (item) =>
            item.correo?.toLowerCase() === correo.trim().toLowerCase() &&
        ...
        if (!estudiante) {
          setErrors({
            ...newErrors,
            correo: "El correo no se encuentra registrado o la contraseña es incorrecta."
          });
          setIsLoading(false);
          return;
        }
        ...
        localStorage.setItem("estudiante", JSON.stringify(usuarioConRol));
        setIsSuccess(true);
      ...
      // ===============================
        ...
        const response = await fetch(`${API_BASE_URL}/api/empresas/login`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          setErrors({
            ...newErrors,
            correo: data.error || "La empresa no se encuentra registrada o la contraseña es incorrecta."
          });
          setIsLoading(false);
          return;
        }
        ...
        localStorage.setItem("estudiante", JSON.stringify(usuarioEmpresa));
        setIsSuccess(true);
        document.body.className = "dashboard-body";
      ...
      setErrors({
        ...newErrors,
        correo: "Error al conectar con la API."
      });
      console.error(error);
      setIsLoading(false);
    }
      ...
      <main className="login-page">
        ...
        <section className="login-hero">
          <div className="hero-content">
            ...
            <p>
              Conecta estudiantes, egresados y empresas con oportunidades laborales,
              permitiendo consultar ofertas, postular y gestionar procesos desde una plataforma web.
            </p>
                ...
                <form className="login-form" id="loginForm" onSubmit={handleSubmit} noValidate>
                  ...
                  <div className="form-field">
                    ...
                    >
                      ...
                      <option value="empresa">Empresa Reclutadora</option>
                    </select>
                  </div>
                  ...
                  <div className={`form-field ${errors.correo ? 'error' : ''}`}>
                    ...
                    <label htmlFor="correo">
                      {rol === "estudiante" ? "Correo institucional" : "Correo de empresa"}
                    </label>
                  ...
                  <div className={`form-field ${errors.password ? 'error' : ''}`}>
                    <input
                      ...
                      name="password"
                      required
                      autoComplete={rol === "estudiante" ? "current-password" : "off"}
                      placeholder=" "
                      value={password}
                    ...
                    <label htmlFor="password">
                      {rol === "estudiante" ? "Contraseña" : "Contraseña de empresa"}
                    </label>
                    <button
                      type="button"
                ...
                <div className="login-note">
                  <p>
                    {rol === "estudiante"
                      ...
                      : "Ingresa con el correo de contacto de la empresa y la contraseña registrada."}
                    {" "}Esta versión corresponde a un prototipo académico conectado a API y Supabase.
                  </p>
                </div>
